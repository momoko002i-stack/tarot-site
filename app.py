...
def show_card_image(image_path, orientation):
    if not image_path.exists():
        st.error(f"画像が見つかりません: cards/{image_path.name}")
        return
...
def show_shuffle_animation():
    st.markdown(
        ...
        .card-a { animation: shuffleA 1.2s ease-in-out infinite; }
        .card-b { animation: shuffleB 1.2s ease-in-out infinite; }
        .card-c { animation: shuffleC 1.2s ease-in-out infinite; }
        @keyframes shuffleA {
...
st.markdown(
    ...
    .meaning-box {
        ...
        border: 1px solid rgba(160,130,80,.25);
        line-height: 1.8;
...
if st.session_state.step == 1:
    ...
    chosen_number = st.slider(
        ...
        0,
        21,
        st.session_state.chosen_number,
    )
    st.markdown(
        """
        <div class="small-note">
        ※ 選んだ数字はカード番号ではありません。カードを開く前の、小さな儀式として扱われます。
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<div class='card-title'>{card['name_en']}</div>"
        f"<div class='card-subtitle'>{card['number']} / {card['name_ja']}</div>"
        f"<div class='center'><span class='orientation'>{orientation}</span></div>",
        unsafe_allow_html=True,
    )
    show_card_image(image_path, orientation)
    st.markdown(
        f"<div class='meaning-box'><strong>このカードのひとつの象徴：</strong><br>{meaning}</div>",
        unsafe_allow_html=True,
    )
    st.markdown("### このカードを見て、何を感じましたか？")
    impression = st.text_area(
        "正解っぽく書かなくて大丈夫です。目に入ったもの、色、人物への印象などを自由に書いてください。",
